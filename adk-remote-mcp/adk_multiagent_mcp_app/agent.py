...
def get_token(tool_context: ToolContext) -> Dict[str, str]:
    """
    ...
    """
    audience = "https://weather-remote-mcp-server-496235138247.us-central1.run.app/mcp/"
    try:
...
def get_gcp_auth_headers(audience: str) -> Dict[str, str]:
    ...
    try:
        ...
        logging.info(
            "Successfully found Google Cloud credentials. Fetching OIDC token."
        )
...
#wea_auth_headers = get_gcp_auth_headers0(wea_url)
        ...
        tools=[
            MCPToolset(
                connection_params=StreamableHTTPConnectionParams(
                    url=wea_url,
                    headers={"Authorization": f"Bearer {ToolContext.state['token']}"},
                )
            )
        ],
        before_tool_callback=get_token,
    )
